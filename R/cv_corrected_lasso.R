...
#' @param radii Optional vector containing the set of radii of the l1-ball onto which the solution is projected.
...
#' cvfit <- cv_corrected_lasso(W, y, sigmaUU) # Run the corrected lasso
#' plot(cvfit)
...
cv_corrected_lasso <- function(W, y, sigmaUU, n_folds = 10, family = "gaussian",
                 radii = NULL, no_radii = 100, alpha = 0.1, maxits = 5000, standardize = FALSE){
  N = nrow(W)
  y <- drop(y)
