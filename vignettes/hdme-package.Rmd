...
### Logistic Regression
At the time of writing, the corrected lasso for logistic regression is implemented in `hdme`. Poisson regression will be added. To illustrate its use, we start by generating some measurement error data with binomially distributed response.
```{r}
set.seed(323)
n <- 2000
p <- 50
...
The plot functions work the same way as for linear regression.
```{r}
plot(corrected_fit)
```
...
corrected_fit <- cv_corrected_lasso(W = W, y = y, sigmaUU = sigmaUU)
```
